...
class BinaryTree {
  ...
  //Der kan addes ved tree.addItem(value)
  addItem(value) {
    let node = this.root;
  ...
  //Kaldes fra root ved tree.dfs(tree.root) på live serverens console
  //Depth first search
  dfs(node) {
    if(!node){
        return;
    }
    //pre-in-post henviser til om output står før, under, efter rejsen/printet videre i træet
    //Pre-order
    /*
  ...
  //Højden kan printes ved tree.root.height
  //Fra slides
  updateHeight(node){
